...
local tpv=util.table.deepcopy(tps)
...
function platform.GetWarpables(c,cf)
	...
	-- Check if there are any players on this surface at all
	local players_on_surface = {}
	for k,v in pairs(game.players) do
		if v.connected and v.surface == c then
			table.insert(players_on_surface, v.name)
		end
	end
	...
	for k,v in pairs(c.find_entities_filtered{type={"car","character","player"},invert=true,area=marea})do
		...
		if(v.type=="spider-vehicle")then
			-- Check if spider vehicle has a driver/passenger - if so, don't add to etbl (clone/destroy list)
			-- It will be handled in the player teleportation logic instead
			local has_player = false
			if v.get_driver and v.get_driver() and v.get_driver().valid then
				has_player = true
			end
			-- Check for passengers too
			if not has_player and v.get_passenger and v.get_passenger() and v.get_passenger().valid then
				has_player = true
			end
			
			-- Only add to clone/destroy list if no players are in the vehicle
			if not has_player then
				table.insert(etbl,v)
			end
		elseif(v.type=="car" or v.type=="tank")then
			-- Check if car/tank has a driver - if so, don't add to etbl (clone/destroy list)
			-- It will be handled in the player teleportation logic instead
			local has_player = false
			if v.get_driver and v.get_driver() and v.get_driver().valid then
				has_player = true
			...
			-- Only add to clone/destroy list if no players are in the vehicle
			if not has_player and (v.last_user or v.force.name=="player" or (biterwarp and v.force.name=="enemy")) then
				table.insert(etbl,v)
			end
	...
	-- find character entities (players) to teleport to new platform
	local characters = c.find_entities_filtered{type="character",area=marea}
	for k,v in pairs(characters)do 
		if v.valid and v.player then
			-- This is a character entity with an associated player
			-- Place players at platform center (0,0) on the new surface
			local spawn_pos = {x = 0, y = 0}
			table.insert(tpply, {v, spawn_pos})
		end
	end
	
	-- Also check for players driving vehicles on the platform
	for k,v in pairs(game.players) do
		if v.connected and v.driving and v.vehicle then
			local vehicle = v.vehicle
			if vehicle.surface == c and vector.inarea(vehicle.position, marea) then
				-- Player is driving a vehicle on the platform - add the vehicle to teleport list
				table.insert(tpply, {vehicle, vector.pos(vehicle.position)})
			end
		end
	end
	...
	for k,v in pairs({"nw","ne","sw","se"})do local ugName="turret-"..v local rHas=research.has("warptorio-"..ugName.."-0") if(rHas)then local ug=research.level("warptorio-"..ugName)
		...
		for a,e in pairs(etp)do 
			if(e.player and e.player.valid and e.player.connected and e.player.character~=nil and not e.player.driving)then 
				-- Add the character entity, not the player object
				table.insert(tpply,{e,{e.position.x,e.position.y}}) 
			end 
		end
